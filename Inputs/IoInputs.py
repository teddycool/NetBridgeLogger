...
try:
    ...
    deviceconfig = {"Button": {"Pressed": 0.1, "LongPressed": 1.5}}
import time
import cv2
    ...
    def update(self):
        ...
        if self._gpio.input(self._inputpin) == False: #Button pressed, PullUp is released
            ...
            if self._lastpress != self._pressed: #First round
                ...
                if time.time() - self._presstime > deviceconfig["Button"]["LongPressed"]:
                    self._state = self._states[2]
                else:
                    if time.time() - self._presstime > deviceconfig["Button"]["Pressed"]:
...
class OnOnSwitch(object):
    ...
    def draw(self, frame, name, x, y):
        cv2.putText(frame,"OnOnSwitch " + name + ": " + str(self._state), (x,y),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255), 2)
        print "OnOnSwitch " + name + ": " + str(self._state)
        return frame
